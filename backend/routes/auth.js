...
// Student Registration (RIGOROUSLY CLEANED PATH)
...
// Login for all roles (RIGOROUSLY CLEANED PATH)
router.post('/login',
    body('email').isEmail(),
    ...
    async (req, res) => {
        try {
            ...
            // Use the constant string 'abc' for signing the JWT (assuming this is intentional)
            const JWT_SECRET = 'abc'; 
            if (!process.env.JWT_SECRET) console.warn('Using hardcoded JWT_SECRET');
...
// General profile route for all users (RIGOROUSLY CLEANED PATH)
